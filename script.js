...
const containers = document.getElementById('imageContainer');
...
function createAnnotations(data) {
  data.forEach((component, index) => {
    ...
    wrappers.classList.add('overlay-image');
    wrappers.classList.add(index % 2 === 0 ? 'right' : 'left'); // Alternance entre droite et gauche
    ...
    wrappers.setAttribute('data-name', component.title);
    wrappers.setAttribute('data-info', component.info);
    wrappers.setAttribute('data-image', component.image);

    const topPercentage = 10 + (70 * index / (data.length - 1)); // Distribution entre 10% et 90%
    wrappers.style.top = `${topPercentage}%`;

    wrappers.appendChild(img);
    containers.appendChild(wrappers);
  });
}
